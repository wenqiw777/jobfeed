...
test("evaluation coverage: shows absolute counts and shares of all discovered jobs", async () => {
  ...
  expect(within(coverage).queryByText("Detailed review")).toBeNull();
...
test("job status uses a logarithmic bar chart so small states remain visible", async () => {
  renderInsights();

  const chart = await screen.findByRole("region", { name: "Job status" });
  expect(within(chart).getByRole("application", { name: "Job status" })).toBeInTheDocument();
  expect(within(chart).getByText(/log scale/i)).toBeInTheDocument();
...
test("the selected range does not render a duplicate activity summary card", async () => {
  ...
  expect(screen.getByRole("region", { name: "Job status" })).toBeInTheDocument();
