"""Ashby ATS vendor adapter for fetching and probing job boards."""
...
async def probe(
    ...
    *,
    ...
    HEAD is unreliable for Ashby; uses GET on the jobs endpoint instead.
    Returns True on 2xx, False on definitive 404/410 misses.

    Args:
        client: Shared async HTTP client.
        slug: Company board slug on Ashby.
    ...
    return await probe_url(
        client, url, slug=slug, vendor=_VENDOR, timeout=timeout, method="GET"
    )
...
def _extract_jobs_list(raw: dict[str, Any] | list[Any], slug: str) -> list[Any]:
    """Extract the jobs list from the Ashby response envelope.
    ...
    if not isinstance(raw, dict) or "jobs" not in raw:
        raise ATSParseError(
            f"Ashby response missing 'jobs' key for {slug}",
            slug=slug,
            vendor=_VENDOR,
        )
    jobs_list = raw["jobs"]
    if not isinstance(jobs_list, list):
        raise ATSParseError(
            f"Ashby 'jobs' is not a list for {slug}",
            slug=slug,
            vendor=_VENDOR,
        )
    return jobs_list
...
def _parse_job(
    ...
    """Parse a single Ashby job object into a JobPosting.
    ...
    try:
        ...
        _logger.warning("Skipping malformed Ashby job for %s: %s", slug, exc)
        return None
...
def _build_posting(
    ...
    """Build a JobPosting from a validated Ashby job object.
    ...
    raw_id = job.get("id")
    if raw_id is None:
        _logger.warning("Skipping Ashby job with null id for %s", slug)
        return None
    ...
    if not canonical_id or not title or not url or not jd_text:
        _logger.warning("Skipping Ashby job with blank required field for %s", slug)
        return None
    ...
    return JobPosting(
        ...
        enrich_source="api-ashby",
    )


def _extract_jd_text(job: Any) -> str:
    """Extract job description text from an Ashby job object.

    Prefers descriptionPlain; falls back to descriptionHtml via html_to_text.
    ...
    html_content = job.get("descriptionHtml") or ""
    return html_to_text(str(html_content)).strip()


def _extract_location(job: Any) -> str:
    """Extract location string from an Ashby job object.
...
def _parse_published_at(published_at: Any) -> datetime | None:
    """Parse Ashby publishedAt timestamp string.

    Args:
        published_at: Raw timestamp value from API.
