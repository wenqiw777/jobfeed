...
- File length: <= 300 lines, enforced as a blocking hygiene gate. Exempt: the `adapters/store/` layer, `cli/migrate.py`, `domain/ml_features.py`, and `adapters/sources/_jobspy.py`. The PostgresStore and legacy-migration surface are inherently large; `ml_features.py` holds the ML-gate vocab name lists plus the compiled regex tables that index them, which must stay in lockstep in one file; `_jobspy.py` is the single JobSpy isolation boundary that keeps lazy imports, timeout/concurrency containment, and DataFrame conversion together so pandas/jobspy details cannot leak into source adapters. Fragmenting any of these into <=300-line shards harms readability. The gate stays blocking everywhere else. (Phase 1 amendment, approved 2026-05-22; `ml_features.py` and `_jobspy.py` added 2026-06-07.)
