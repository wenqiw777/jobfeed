...
| `application_stats(since_days_ago, by_resume)` | 返回 applied/response/interview/offer/rejection counts、median days，可选 by-resume。无 window applications 时全 0、median None、by_resume None。 | 多个 SELECT，无 snapshot transaction。 | window population 来自 history 的 `to_status='applied' AND changed_at>=cutoff`，不是 `applied.applied_at`。response 为这些 jobs 历史上曾到达 interviewing/offer/rejected 的 distinct job 数；interview count 包含 reached interviewing **或 offer**；median 用各 job 全历史最早 applied 到最早 response 的整日差。variant NULL→`unknown`。同 job 多条 window applied history 时 variant 选择因 query 无 ORDER 当前未定义。 | `tests/contract/test_store_contract.py::TestApplicationAudit::test_application_stats_*`；`tests/contract/test_status_apply_persistence.py::TestEdgeCases::test_application_stats_zero_applications`；`tests/unit/test_application_service.py::TestStats`。median/outcome edge coverage 不足，见 7.2。 |
...
| `list_upcoming_interviews(within_days)` | 返回 scheduled、open 且 parent status 正为 interviewing 的 rounds。 | 单条只读。 | 条件只有 `scheduled_at <= now + within_days`，**没有 `scheduled_at >= now`**，因此过期但仍 open 的 round 也会返回。NULL scheduled 排除；completed 排除；排序 `scheduled_at ASC`，tie 未定义。 | `tests/integration/test_interview_rounds_store.py::test_list_upcoming_interviews` 与 `test_upcoming_excludes_completed`。过去时间和非-interviewing 排除未直接测试；无 production call site，见 7。 |
...
- `application_stats` 的 median、response-before-window、同 job 多 applied history
  与 by-resume variant 冲突。目前最后一项没有 ORDER，行为不可确定。
...
- `list_upcoming_interviews` 对 past round 和 parent 非-interviewing 的直接断言。
