...
| `application_stats(since_days_ago, by_resume)` | 返回 applied/response/interview/offer/rejection counts、median days，可选 by-resume。无 window applications 时全 0、median None、by_resume None。 | 多个 SELECT，无 snapshot transaction。 | window population 来自 history 的 `to_status='applied' AND changed_at>=cutoff`，不是 `applied.applied_at`。response 为这些 jobs 历史上曾到达 interviewing/offer/rejected 的 distinct job 数；interview count 包含 reached interviewing **或 offer**；median 用各 job 全历史最早 applied 到最早 response 的整日差。同 job 多条 window applied history 时，以最小 history ID 的首次 apply variant 为准；variant NULL→`unknown`。 | `tests/contract/test_store_contract.py::TestApplicationAudit::test_application_stats_*`；`tests/contract/test_status_apply_persistence.py::TestEdgeCases::test_application_stats_zero_applications`；`tests/unit/test_application_service.py::TestStats`。median/outcome edge coverage不足，见 7.2。 |
...
- `application_stats` 的 median 与 response-before-window；同 job 多 applied history 的
  variant 已冻结为最小 history ID 对应的首次 apply variant。
