...
| `application_stats(since_days_ago, by_resume)` | 返回 applied/response/interview/offer/rejection counts、median days，可选 by-resume。无 window applications 时全 0、median None、by_resume None。 | 多个 SELECT，无 snapshot transaction。 | window population 来自 history 的 `to_status='applied' AND changed_at>=cutoff`，不是 `applied.applied_at`。response 为这些 jobs 在最早 applied 时间之后（严格 `>`）到达 interviewing/offer/rejected 的 distinct job 数；interview count 包含 reached interviewing **或 offer**；median 用各 job 最早 applied 到其后最早 response 的整日差。同 job 多条 window applied history 时，以最小 history ID 的首次 apply variant 为准；variant NULL→`unknown`。 | `tests/contract/test_store_contract.py::TestApplicationAudit::test_application_stats_*`，包含 fixed median、pre-apply exclusion 与 variant ordering；`tests/contract/test_status_apply_persistence.py::TestEdgeCases::test_application_stats_zero_applications`；`tests/unit/test_application_service.py::TestStats`。 |
