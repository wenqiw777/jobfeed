...
| `save_stage_b_error` (`store.py:107-114`; `postgres.py:2538-2558`) | Existing job ID + error → `None`; missing job raises FK error | **W**; deliberately non-idempotent error count | Sets status `error`, last error, increments count and time exactly once per call. Direct repeat/latest-error/FK evidence: `test_evaluation_claim_release_contract.py`; other evidence: `test_store_contract.py:395-402`. |
...
| `release_stage_a_claim` (`store_claims.py:144-150`; `postgres.py:2921-2939`) | Decimal job ID → `None`; malformed raises; missing/not-in-progress is no-op | **W**, idempotent after first release | Only `in_progress` changes. Restore `error` if error text exists, else `completed` if score exists, else NULL; update time only on the first release. Every restoration/missing/replay branch is covered by `test_evaluation_claim_release_contract.py`. |
| `release_stage_b_claim` (`store_claims.py:152-158`; `postgres.py:2941-2959`) | Decimal job ID → `None`; malformed raises; missing/not-in-progress is no-op | **W**, idempotent after first release | Restore `error` if error text exists, else `completed` if verdict exists, else NULL; update time only on the first release. Every restoration/missing/replay branch is covered by `test_evaluation_claim_release_contract.py`. |
...
| C07 | `save_stage_b_error` repeat-count and release Stage A/B restoration branches were incomplete | **RESOLVED:** direct tests cover exact error increments/latest detail/FK failure plus error/completed/NULL restoration, missing-row no-op, and replay timestamp idempotency for both claim stages. |
