...
| `insights_overview` | caller-validated window -> overview | Totals and distributions are all-time. `evaluated_jobs` means non-NULL `stage_a_at`; gate count means result `pass`; verdict wins, otherwise only skipped-below-threshold enters derived bucket. Daily buckets are UTC and closed on both ends `[now-days, now]`, omit future/NULL timestamps, omit empty days, merge three measures with zeros, and order day ASC. |
