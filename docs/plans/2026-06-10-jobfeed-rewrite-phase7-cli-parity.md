...
**Implementation repo:** `/Users/wenqiwang/wwq/jobfeed` (NOT legacy `job-apply`). Bare paths below are rooted there. This file (`docs/plans/2026-06-10-jobfeed-rewrite-phase7-cli-parity.md`) is the Phase 7 source of truth.
**Commit strategy:** one conventional commit per task, no AI attribution.
**Execution mode:** Sequential.
**Per-task quality gate (matches GitHub CI):** `make quality` (ruff + format check + mypy + unit/contract tests) AND `pytest -m postgres -o "addopts=" -v` must both pass before the task's commit. The CI browser/docker lanes are unaffected by Phase 7 tasks. Never run `@mlmodel` locally.

> **History:** Design drafted 2026-06-10, revised after 1 independent agent review against both codebases. The review corrected the effort map (bootstrap-companies is ~220 lines with ONE generic parser, not 7; the ATS probe is already a shared module) and surfaced the two real MUSTs the draft missed: digest file output + auto-cutoff persistence, and `list` output/default parity. This file supersedes the deleted standalone spec. Second review 2026-06-10 (3-agent claim-by-claim verification against both repos) corrected: `StatusInfo` already has `last_status_change_at` (T2 adds only company/title); `ResumeSnapshot.source` is already stamped at apply time, so D7 uses the stored column instead of JOIN-derived source; `DigestService` must widen its store protocol to reach `workflow_attention`/`needs_attention`/KV (T4); `companies add --ats` sets `ats_override=True`; `enrich-paste` platform choices pinned to linkedin|indeed; bootstrap regex/line cites made exact.
...
- Companies store layer is complete: `upsert_company/get_company/list_companies/mark_company_removed` (`ports/store_ops.py:21-66`, implemented in `postgres.py:4110-4216`). `list_companies` already takes `vendor:`/`include_removed:`; soft delete is the `ats_vendor='removed'` sentinel; `CompanyRecord` carries `ats_override` (set it True on the `--ats` pin path, legacy parity); `mark_company_removed` returns bool.
...
- Store KV `get_state/set_state` exists (`ports/store_ops.py:128-146`; `state` table is in migration 0001) — the vehicle for digest auto-cutoff.
- `ResumeSnapshot.source` is already stamped `master`/`tailored` at apply time (`services/application.py:194-206`); `save_resume_snapshot` is `ON CONFLICT DO NOTHING`, so a hash keeps its first-capture source.
- Capability split that Task 4 must respect: KV + `needs_attention` + `enrich_paste` live in `StoreOpsMixin`; `workflow_attention` lives in `StoreStatusMixin`; core `JobStore` (what `DigestService` takes today) has none of them.
...
- Phase 6 planned `set_followup`/`followup --in` but they did NOT ship: no `set_followup` in `ports/store_status.py` or `postgres.py`, no `followup` command registered. (Legacy has no store method either — it inlines a raw UPDATE, legacy `main.py:1698-1718`.)
...
- **D7 — Snapshot identity supports hash prefixes; global list uses the stored `source` column + a JOIN-derived usage count.** `get_resume_snapshot_by_prefix(prefix)` errors on ambiguity (≥2 matches) and on no-match. `list_resume_snapshots()` returns every snapshot with its stored `source` (`master`/`tailored` — already stamped by the apply flow, see A2) and a usage count = number of `applied` rows referencing the hash as `master_resume_hash` OR `tailored_resume_hash` (LEFT JOIN; orphans show usage 0 and still appear). `--source` filters on the stored column. Deriving source from joins was considered and dropped — the column already exists and is populated.
...
- **D9 — Attention ships inside digest; reapply notice ships inside `apply`.** Digest gains a footer: workflow attention (`follow_up_today` / `interview_prep` / `going_ghosted` buckets) + pipeline `needs_attention`. `apply` prints `compute_reapply_notice` when non-None. This supersedes Phase 6 A8's "digest fold = Phase 8+" deferral — the data methods exist with zero callers; the wiring is small and is exactly what daily dual-running needs. (Legacy materialized the notice during `evaluate` and surfaced it in the digest; the rewrite computes it on demand at apply time — `compute_reapply_notice` excludes the job itself via `j.id != $2`, so the post-apply call is safe.)
...
| `enrich-paste <canonical_id> --platform linkedin\|indeed [--from-file PATH]` | NEW; stdin when no file |
...
**What to build:** (1) `set_followup(job_id: int, at: datetime) -> bool` — UPDATE `job_status.next_followup_at`; returns False when no `job_status` row exists. (2) Switch `notes_contain` to `ILIKE` (currently `LIKE`, `postgres.py:3057`; D6). (3) Add `company`, `title` to `StatusInfo` — `last_status_change_at` is ALREADY a field and already selected; extend the `list_statuses` SELECT (it already JOINs `jobs` at `postgres.py:3064` without selecting `j.*`) to populate the two new fields. (4) Add `LIST_DEFAULT_STATUSES = STATUS_VALUES - {"new","archived"}` to `domain/status.py` (D5; store semantics unchanged — `None` still means no filter).
**Acceptance:**
...
**What to build:** Per D7. (1) `get_resume_snapshot_by_prefix(prefix: str) -> ResumeSnapshot` — `WHERE resume_hash LIKE prefix || '%'`; raise a domain-level `LookupError` subclass distinguishing no-match from ambiguous (≥2). (2) `list_resume_snapshots(source: str | None = None) -> list[...]` — every snapshot with `captured_at`, its stored `source` column, and a usage count (number of `applied` rows referencing the hash as `master_resume_hash` OR `tailored_resume_hash`, via LEFT JOIN; orphans count 0 and still appear); `source` filters on the stored column. (3) `list_applications(limit, resume_hash_prefix: str | None = None)` — filter on either hash column matching the prefix.
**Acceptance:**
- [ ] Unique prefix resolves; ambiguous prefix and unknown prefix raise the two distinct errors
- [ ] A snapshot referenced as master by 2 jobs lists usage 2 / source `master`; a tailored one lists source `tailored`; an orphan (zero `applied` references) lists usage 0 and still appears
...
**What to build:** Per D2/D3/D4/D9. Domain: render accepts `top: int | None`, caps each verdict group at N with `(shown/total)` in the group header. Service: widen the store protocol first — `DigestService` takes core `JobStore` today, which has none of the needed capabilities; define `DigestStore(JobStore, StoreOpsMixin, StoreStatusMixin, Protocol)` in `services/digest.py` (the `WorkflowStore` pattern, `services/workflow.py:33`; the CLI casts like `cli/status.py:20`). Then: when `cutoff_at` is None read KV `digest.last_rendered_at` as the cutoff; after a successful render write `now()` to that key. Accept an optional `output_dir: Path | None`; when set, write `today.md` (overwrite) and `YYYY-MM-DD.md` (today's date, overwrite). Append a footer section rendering `store.workflow_attention()` (three buckets, store defaults) and `store.needs_attention()` (existing defaults), each omitted when empty. Config: `DigestSettings.output_dir: str | None = None`, `[digest]` section in `config.example.toml` (commented example pointing under `~/.cache/jobfeed/`). CLI: `--top N` (≥1), `--quiet` (suppress stdout echo; usage error without configured output_dir); resolve output_dir from settings (service stays config-free).
**Acceptance:**
...
**What to build:** Click group `companies`. `add <slug> [--ats greenhouse|ashby|lever]` (choices = `SUPPORTED_VENDORS`, `adapters/sources/ats.py:28`): with `--ats`, upsert directly with `ats_override=True` (pins the vendor against re-probe, legacy parity); without, build an httpx AsyncClient in-command (Phase 2 pattern) and call `probe_company`; unresolvable probe → clear error, no row written. Re-adding a previously removed slug restores it (upsert overwrites the `removed` sentinel). `list [--vendor greenhouse|ashby|lever] [--include-removed]` printing slug/vendor/failure-count (`consecutive_discover_failures`); removed rows carry the `ats_vendor='removed'` sentinel and surface only with `--include-removed`. `remove <slug>` → `mark_company_removed`; report when slug didn't exist (method returns bool).
**Acceptance:**
- [ ] `add --ats greenhouse` writes without network (row carries `ats_override=True`); probe path resolves a vendor or errors cleanly without writing; re-adding a removed slug restores it
...
**What to build:** Per D8. Pure module: `BOOTSTRAP_SOURCES` (port the 7-entry name→raw-README-URL dict from legacy `main.py:2140-2175`), `extract_ats_slugs(markdown) -> set[tuple[slug, vendor]]` (iterate the 3 `ATS_SLUG_PATTERNS` regexes ported verbatim from legacy `main.py:2180-2186` — note the greenhouse pattern covers `(?:job-)?boards(?:\.eu)?\.greenhouse\.io`; lowercase + dedupe slugs), `extract_ats_slugs_with_age(markdown, max_age_days)` (age-aware variant: legacy `main.py:2238-2260` scans `<tr>` rows and regex-matches the `Nd`/`Nmo` age cell, `mo` = 30 days; legacy returns raw ages and the caller filters — here the function takes `max_age_days` and filters itself, rows with no parseable age pass through). CLI command `bootstrap-companies [--source <name>|all] [--apply] [--max-age-days N]`: fetch via lazily-built httpx client, parse, diff against `list_companies`, print add/skip summary; write upserts only with `--apply`.
**Acceptance:**
...
**What to build:** (1) `followup <job_id> --in 7d` → Task 1 parser → `WorkflowService.set_followup` (new passthrough) → Task 2 store method; not-found exits nonzero. (2) `enrich-paste <canonical_id> --platform [linkedin|indeed]` (default `linkedin`, legacy parity) `[--from-file PATH]`; no file → read stdin; empty text → usage error; calls `store.enrich_paste` (returns the job id only); print the job id plus the quality from `domain.quality.assess_quality(jd_text)` (the same function the store applies internally). (3) `list`: add `--days` (Task 1 back-parser), `--notes-contain`, `--limit`, `--allow-empty`; default statuses = `LIST_DEFAULT_STATUSES` when `--status` absent; output columns id/status/company/title/last_change/followup (plain, `--md`, `--json` all carry the new fields); empty result exits 1 unless `--allow-empty` (D5). (4) `apply`: `--method` choice flag mapping to the existing `ApplyRequest.application_method`; `--notes` — add field to `ApplyRequest`, plumb into `ApplicationRecord.notes`; after a successful apply, print `compute_reapply_notice` when non-None (D9). (5) `apply-history --resume <prefix>` → Task 3 filter.
**Acceptance:**
...
- **Type consistency:** `set_followup(int, datetime)->bool` (T2→T7), `get_resume_snapshot_by_prefix`/`list_resume_snapshots(source)`/`list_applications(limit, resume_hash_prefix)` (T3→T7/T8), `parse_window`/`parse_window_back` (T1→T7), `LIST_DEFAULT_STATUSES` (T2→T7/T8), digest `top`/`output_dir`/KV key `digest.last_rendered_at` + `DigestStore` protocol (T4→T8).
