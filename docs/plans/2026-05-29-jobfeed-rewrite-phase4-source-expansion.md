...
> **Revision 2 (2026-06-07 hardcoded-audit cleanup — OVERRIDES conflicting text below):** SpeedyApply no longer has a production built-in default README; enabling it requires explicit `search_urls`, with the 2026 SpeedyApply list shown only as an example config value. LinkedIn in-source priority no longer treats the literal year `"2026"` as a fall-intern signal; ranking uses semantic terms such as `intern` and `fall` from the title/source URL. JobSpy sources have explicit timeout/concurrency settings, and Indeed country is configurable. Historical Decision/Task text below that says empty SpeedyApply URLs select a built-in default or that LinkedIn priority checks `"2026" in title` is superseded.
