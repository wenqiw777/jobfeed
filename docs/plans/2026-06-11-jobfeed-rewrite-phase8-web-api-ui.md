...
`GET /jobs` semantics: `sort ∈ {discovered_desc (default), posted_desc, score_desc, company_asc}`, honored by Library tabs only — `queue`/`pending_jd` always use the fixed verdict-group order (T3). `apply_hard_filters` and `dedupe` default to false (the raw endpoint is neutral; the frontend's typed param builders set them per zone). `tab_counts` carries all six tab values on every response, each computed in SQL with that tab's own predicate plus the request's shared `search`/`posted_within_days` filters; the requested tab's `total` is the post-fold/hard-filter true total used for pagination. The response field `jobs` is the DTO name for `JobsViewPage.rows`.
