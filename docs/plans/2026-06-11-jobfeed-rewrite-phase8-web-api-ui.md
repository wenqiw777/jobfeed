...
**Prerequisite:** this plan (+ `PRODUCT.md`/`DESIGN.md`) currently lives on `feat/linkedin-guest-source` — merge that branch into `main` first; Phase 8 branches off `main`.

> **History:** Designed 2026-06-11 via brainstorm (visual directions + IA explored in-browser with the user; all decisions user-confirmed). Code verification on 2026-06-11 corrected the standalone spec in two places (see D6, A2); this file supersedes the deleted standalone spec (`docs/superpowers/specs/2026-06-11-jobfeed-rewrite-phase8-web-api-ui-design.md`). A second code-verification pass on 2026-06-11 corrected counter/status/predicate details (A2 corrections block, D9/D10 amendments, new D13–D14). Companion product/visual docs: `PRODUCT.md`, `DESIGN.md`.
...
- Phase 7 CLI surface is merged and registered: `followup`, `companies` (add/list/remove), `bootstrap_companies`, `enrich_paste`, `interview`, `snapshots`, plus list/digest/apply upgrades (`cli/__init__.py:230-250`).
- App assembly is reusable: `create_app(config_path) -> AppContext` (`cli/__init__.py:35`) and `_create_store(settings)` (`cli/__init__.py:209`) do config→adapters→services wiring; the web app factory wraps the same path.
- Companies store layer complete incl. bulk: `upsert_company/get_company/list_companies/mark_company_removed` (`postgres.py:4280-4366`) + `bulk_insert_companies` (`postgres.py:1677`). No boolean removed column — soft removal is encoded as `ats_vendor='removed'` (the API's removed flag is derived); the failure count column is `consecutive_discover_failures`. ATS probe shared at `adapters/sources/_ats_probe.py` (`probe_company(client, slug, *, timeout)`, PROBE_ORDER greenhouse/ashby/lever, raises `ProbeNetworkError`/`ProbeIndeterminateError`, no DB writes).
- Workflow/application services cover every write the web needs: `transition/transition_bulk/restore/note/set_followup/add_round/list_rounds/complete_round/attention/auto_decay` (`services/workflow.py`; the longer `*_interview_round` names live on the store layer, not the service), `apply/apply_history/stats(by_resume=True)/get_snapshot/get_snapshot_by_prefix/list_snapshots/diff_snapshots` (`services/application.py`). Error surface for HTTP mapping: illegal transition raises `ValueError`; missing status row raises `KeyError`; `set_followup` returns `False` (no exception) when there is no status row; `complete_round` raises `ValueError("no open interview round…")` for unknown *and* already-completed indexes alike. `ApplyRequest` fields are **content-based strings** (master/tailored resume, cover letter — `services/application.py`), so multipart uploads map directly; the web route mirrors `cli/apply.py`'s request assembly (incl. verdict/fit/hooks snapshots, reapply notice).
...
- `get_pipeline_run(run_id)` / `record_pipeline_run` exist (`postgres.py:2773-2819`); `PipelineRun` is a **flat counters object** (`domain/models.py:184`: run_id, started_at, source, jobs_discovered/inserted/updated/filtered/ml_gated, stage_a_scored, stage_b_scored, jobs_scored, total_llm_cost_usd, errors, finished_at — plus a non-counter `dry_run_preview` list the Runs zone ignores).
- Attention trio wired by Phase 7 with CLI callers: `workflow_attention` (`ports/store_status.py:115`), `needs_attention` (`ports/store_ops.py:257`), `compute_reapply_notice` (a store-port method, `ports/store_status.py:132`, wrapped by `ApplicationService.reapply_notice`).
...
- Jobs listing today is thin (`list_jobs(limit)`, `list_evaluated_jobs(limit)`, `list_statuses(StatusFilter | None)`); the web's filtered/folded/paginated jobs view is **net-new** (D10).

**Corrections from the second verification pass (code wins):**
- **The status set is 11 values** (`domain/status.py:5`): new, scored, shortlisted, awaiting_referral, applied, interviewing, offer, rejected, ghosted, archived, ignored. There is **no `closed` status** — `jobs.closed_at` is a job-liveness column, not a workflow status. `ALLOWED_TRANSITIONS` lives in `domain/status.py:21` (not models_status.py). Zone coverage for all 11 is pinned in D13.
- **The Verdict enum is apply/consider/skip only** (`domain/models.py:26`). "Below threshold" is not a verdict — it is derived from `evaluations.stage_b_status = 'skipped_below_threshold'`; the T3 sort treats it as a derived fourth group.
- **`BulkResult` has no `cascaded` counter today** (`domain/models_status.py:77`: `succeeded: int`, `failed: list[(job_id, error)]`, `skipped: int`). The twin cascade happens (`expand_twin_ids` + cascade reason) but is not counted; T4 adds `cascaded: int = 0` additively (D14).
- **`enrich_paste` is a store method, not a service** (`ports/store_ops.py:151`, impl `postgres.py:4544`), keyed by platform+canonical_id, returning the job_id — not the assessed quality. The web route re-reads the job for the response's quality.
- **The JD quality enum is `QualityBand`** (`domain/models.py:15`: full/good/partial/stub/missing/abandoned); `domain/quality.py` holds char thresholds + `assess_quality`. There is no "inadequate" constant — the pending-JD quality predicate is `jd_quality IS NULL OR jd_quality IN ('missing','abandoned')` (same predicate as the stale-close SQL at `postgres.py:4821`; rewrite-spec §15 prose says "missing/abandoned" — the `IS NULL` case is a deliberate code-derived addition covering never-assessed rows).
- **No stored final score.** Scores are `stage_a_score` (0–100) and Stage B `fit_analysis.score`; "final score" below always means the Stage B fit score. Stage B hook fields live in `raw_blocks`/`hooks_json` under keys `lead_with`/`supporting`/`avoid_mentioning` (the normalized `StageBResult.resume_hooks: list[str]` collapses them).
...
- **D13 — Status→zone coverage is total (no orphan statuses).** Triage·Queue statuses = {new, scored, shortlisted, awaiting_referral} (awaiting_referral is a pre-apply waiting state — it stays in the decide loop, where the apply action happens). Pipeline groups = applied / interviewing / offer / Closed, where Closed is a **display group** = {rejected, ghosted}. Library tabs: Shortlisted = {shortlisted, awaiting_referral} (badge distinguishes), Archived = {archived, ignored}; All = everything. Each of the 11 statuses has a home.
- **D14 — `BulkResult` gains `cascaded: int = 0` (additive).** The store bulk path already computes the cascade set (`expand_twin_ids` + cascade reason) — Phase 8 counts it where cascade transitions are applied. The default keeps existing CLI output and Phase 6/7 tests untouched; the web bulk endpoint then returns the four counters for the toast.
...
| `GET /jobs/{id}` | detail pane/drawer | job + evaluation (Stage A score + Stage B fit score + Stage B blocks) + status/history/notes + twins(platform+status) + interviews + snapshot refs |
| `POST /jobs/{id}/transition` | status actions | `{to, note?, force?}`; illegal transition → 409 |
| `POST /jobs/bulk/transition` | bulk bar | `{items:[{id,to}], force?}` → `{succeeded, skipped, failed:[{id,error}], cascaded}` (D14) |
...
`GET /jobs` semantics: `sort ∈ {discovered_desc (default), posted_desc, score_desc, company_asc}`, honored by Library tabs only — `queue`/`pending_jd` always use the fixed verdict-group order (T3). `apply_hard_filters` and `dedupe` default to false (the raw endpoint is neutral; the frontend's typed param builders set them per zone). `tab_counts` carries all six tab values on every response, each computed in SQL with that tab's own predicate plus ALL the request's shared filters (search, posted_within_days, statuses, require_verdict) — invariant: `tab_counts[t]` equals the SQL-prefilter total of the same request with `tab=t`; sidebar/global counts therefore come from requests without statuses/require_verdict narrowing. The requested tab's `total` is the post-fold/hard-filter true total used for pagination. The response field `jobs` is the DTO name for `JobsViewPage.rows`.

**Web zones (semantics inherited from rewrite-spec §15):** Triage·Queue = legacy `/today` semantics (status new/scored/shortlisted/awaiting_referral per D13, hard filters, freshness incl. tier-aware non-apply gate, not closed per D10, verdict sort, fold, decide→auto-advance). Triage·Pending JD = legacy Pending-JD tab semantics (jd_quality NULL/missing/abandoned, no Stage A score, excl. archived/ignored, not closed, hard filters + freshness; paste returns row to the scored flow). Pipeline = `/applied` ∪ `/followups` + attention bar + interview rounds. Library = `/all` minus Pending JD, tabs All/Scored/Shortlisted/Archived, search/sort/pagination, counts sync. Insights = `/stats` views (KPI, funnel, donut, timeline, by-resume). Runs/Sources per D6/D1.
...
**What to build:** (1) `JobsViewQuery` (statuses, tab, search, posted_within_days, require_verdict, limit, offset) / `JobsViewRow` (job core + company_norm/title_norm + status + verdict + stage_a_score + Stage B fit score + posted_at/discovered_at/closed_at + jd_quality) / `JobsViewPage` (rows, total, tab_counts). (2) `query_jobs_view` in postgres: one SELECT joining jobs+evaluations+job_status with SQL predicates for statuses / freshness / case-insensitive search on company+title / closed-row exclusion on Triage tabs (D10) / the pending-JD predicate (`jd_quality IS NULL OR jd_quality IN ('missing','abandoned')` AND no Stage A score AND status ∉ {archived, ignored} — same quality predicate as `postgres.py:4821`, §15 semantics); returns bounded row sets plus per-tab counts (all six tab values, each tab's own predicate + the request's shared search/freshness filters, per A4). (3) `pick_display_representatives(rows) -> list` in `domain/dedupe.py` per D9 — pure, status-priority class ({applied, interviewing, offer} > {shortlisted, awaiting_referral} > rest) ahead of the existing `_representative_sort_key`; `pick_representatives` untouched.
**Acceptance:**
- [ ] Pending-JD predicate pinned with one fixture row per exclusion reason (quality ≥ stub, e.g. 'good' / has Stage A / archived / ignored / closed)
...
**What to build:** Service composes: store `query_jobs_view` → optional hard filters (`domain/filtering.py`) → optional fold (D9) → verdict-group sort (apply → consider → skip per `stage_b_verdict`, then a derived below-threshold group from `stage_b_status='skipped_below_threshold'`; Stage B fit score desc within groups with `stage_a_score` fallback, unscored last) → in-memory pagination with true totals (D10). `GET /api/jobs` maps query params to `JobsViewQuery` + flags (`apply_hard_filters`, `dedupe`, defaults false); `sort` honored on Library tabs per A4, queue/pending_jd keep the fixed verdict-group order; `schemas.py` maps `JobsViewPage.rows` to the response field `jobs`. `GET /api/jobs/{id}` aggregates detail via `get_job`/`get_evaluation`/`get_status`/`get_status_history`: job; evaluation (Stage A: `stage_a_score` + `one_line`; Stage B: `verdict`, `jd_summary`, `fit_analysis.score/strengths/gaps`, and hooks read from `raw_blocks`/`hooks_json` keys `lead_with`/`supporting`/`avoid_mentioning` — the normalized `resume_hooks: list[str]` collapses them); status + history + notes; twins (same company_norm+title_norm, each with platform + status); interview rounds; snapshot refs from the application record. DTOs in `schemas.py` (split into a package if >300 lines).
**Acceptance:**
- [ ] Queue-tab request (statuses new/scored/shortlisted/awaiting_referral + hard filters + fold + require_verdict) returns the legacy-Today-equivalent set for a seeded fixture matrix
- [ ] Ordering pinned: verdict groups in order incl. the derived below-threshold group, score-desc inside
...
**What to build:** Transition (single: `{to, note?, force?}` → `WorkflowService.transition`; illegal per `ALLOWED_TRANSITIONS` (`domain/status.py`) raises `ValueError` → 409 with code `illegal_transition`; missing status row raises `KeyError` → 404). Bulk: `{items, force?}` → `transition_bulk` → `{succeeded, skipped, failed:[{id,error}], cascaded}`; this task **adds `cascaded: int = 0` to `BulkResult`** per D14 (additive field in `domain/models_status.py`, counted in the postgres bulk path where cascade transitions are applied; existing CLI output and Phase 6/7 tests unaffected). Note, followup (ISO datetime, D12; `set_followup` returns `False` when there is no status row — route maps that to 404). JD paste: `{text}` → store `enrich_paste` (`ports/store_ops.py:151` — a store method, not a service) keyed by the job's platform+canonical_id (route resolves them via `get_job`); empty text → 422; `enrich_paste` returns the job_id, so the route re-reads the job and returns its stored `jd_quality` as the assessed quality. Interviews: GET list / POST add `{label, scheduled_at?}` / PATCH `{index}` complete `{notes?}` — service methods are `add_round`/`list_rounds`/`complete_round` (`add_round` auto-transitions applied→interviewing); unknown or already-completed index raises `ValueError("no open interview round…")` → 404. Apply: multipart per D8 — uploads decoded UTF-8, master resume content resolved at the boundary from settings (same source as `cli/apply.py`), evaluation-derived snapshot fields, `{variant, method, notes}` form fields; response includes the reapply notice when non-None; re-applying an applied job returns the existing no-op parity result. Applications history: `GET /api/applications?limit=` → `ApplicationService.apply_history(limit=…)` rows as DTOs (`web/routes/applications.py`).
**Acceptance:**
- [ ] Illegal transition → 409 + code; legal one persists and shows in `GET /jobs/{id}` history
- [ ] Bulk over a twin cluster returns non-zero `cascaded` and the twin's status actually changed (`@postgres`); existing Phase 6/7 status/bulk tests stay green after the additive `BulkResult` change
...
**What to build:** Per D7. Dump script renders the factory's OpenAPI schema deterministically (sorted keys) to the committed snapshot; contract test compares live vs committed and fails with "run `make web-schema` and review the diff". Static serving: when `web-ui/dist/` exists, serve it at `/` with SPA fallback (unknown non-`/api` paths → `index.html`); `/api/*` 404s stay JSON. Also in this commit: fold the zone-mapping amendment (A4 table) into rewrite-spec §15 (`docs/specs/2026-05-20-jobfeed-rewrite-design.md`, "15. Web UI Feature Parity Contract"), and add the Phase 6/7/8 plan entries to CLAUDE.md's phase-plan list (currently stops at Phase 5).
**Acceptance:**
...
**What to build:** Library: full-width virtualized table, tabs All/Scored/Shortlisted/Archived with live counts (Shortlisted = {shortlisted, awaiting_referral}, Archived = {archived, ignored} per D13; closed rows included with a badge per D10), debounced search, sort dropdown, server-side pagination (typed param builders); row click opens a Sheet drawer hosting the same DetailPane. Insights: recharts components styled from tokens over `GET /api/insights/overview` — KPI cards, funnel (discovered → gated → Stage A → Stage B → apply-verdict → applied), status donut, daily timeline, by-resume table.
**Acceptance:**
...
## Self-Review
- **Spec coverage:** D1/D6 scope → T5 (runs read-only) + T6/T12 (sources full, probe flow); D2 → T1 (loopback guard); D3/D11 → T8; D4 → T8/T9 tokens + T12 detector; D5 zone semantics → T2 (predicates) / T3 (queue parity) / T9–T11 (zone UIs) + §15 docs amendment in T7; D7 → T7/T8; D8 → T4; D9/D10 → T2/T3; D12 → T4/T9; D13 (status coverage) → T2/T3/T10/T11; D14 (cascaded counter) → T4/T9. Phase 4/5 display-fold debt → T2; Phase 6/7 attention debt → T5/T10. All seven spec acceptance items land in T12's sweep.
- **Placeholders:** none; every criterion test-verifiable; Stage-B block field names and the quality predicate are pinned to exact code identifiers (A2 second-pass corrections).
- **Type consistency:** `query_jobs_view`/`JobsViewQuery/Page` (T2→T3), `pick_display_representatives` (T2→T3), `list_pipeline_runs`/`insights_overview`/`InsightsOverview` (T5), error shape `{error:{code,message,request_id}}` (T1→T3/T4/T6), four bulk counters incl. additive `cascaded` (D14, T4→T9), ISO followup (T4→T9), openapi.json path (T7→T8), density 32/46px (T8→T9).
- **Corrections honored:** no pipeline_steps/source_health invented (D6); ApplyRequest content-based confirmed before T4 was written; scoring-side `pick_representatives` untouched (Phase 5 contract guard); second verification pass folded in (11-status set + D13 zone coverage, derived below-threshold group, additive `cascaded` counter D14, store-level `enrich_paste` with job re-read for quality, exact pending-JD predicate, service-level interview method names, error→HTTP mappings, `closed_at` exclusion in Triage).
