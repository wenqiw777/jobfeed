...
- **`ports/`** — Protocol interfaces. Imports domain only. `JobStore` (`store.py`), `StoreOpsMixin` (`store_ops.py`), evaluation-batch/claim extensions (`store_ext.py`, `store_claims.py`), the application-audit capability (`StoreApplicationMixin` in `store_application.py`), the status/workflow capability (`StoreStatusMixin` in `store_status.py`), `LLMClient` (`llm.py`), `PromptRenderer` (`prompts.py`), `SimpleSource` (`source.py`).
